...
import { addTask, deleteTask } from "../redux/actions/habitAction";
import './Todo.css';
...
function Todo() {
    ...
    return(
        <>
         <div className="child-div text-center">
          <figure>
            <h5 className="subTitle">Add New Task Here 🕊</h5>
          </figure>
        </div>
  ...
  {inputTask.length === 0 || inputDesc.length === 0 ? (
    ...
    <button
      ...
      onClick={() => {
        dispatch(addTask(inputTask, inputDesc));
        setInputTask('');
        setInputDesc('');
    ...
    >
      <i className="fa-solid fa-pen-to-square"></i> Add The Task
        ...
        <button className="delete-button" onClick={() => { dispatch(deleteTask(elem.id)); alert("Successfully Deleted"); }}>
        Delete
      </button>
        </div>
        )  
